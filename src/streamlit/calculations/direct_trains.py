    ...
    for station in list(direct_train_dict.keys()):
        for station2 in list(direct_train_dict[station].keys()):
            if not direct_train_dict[station][station2]:  # if list is empty
                ...
                direct_train_df = direct_train_df.fillna(0)
                direct_train_df = (
                    direct_train_df.groupby("train_name")
                    .agg(
                        {
                            "ride_time_with_delay": lambda x: round(x[~direct_train_df["is_canceled"]].mean(), 2),
                            "departure_time_delta": lambda x: round(x[~direct_train_df["is_canceled"]].mean(), 2),
                            "arrival_time_delta": lambda x: round(x[~direct_train_df["is_canceled"]].mean(), 2),
                            "is_canceled": lambda x: round(x.mean() * 100, 2),
                        }
                    )
                ...
                with (save_dir / "direct_trains" / file_name).open("w") as f:
...
if __name__ == "__main__":
    ...
    with (save_dir / "direct_train_overview.json").open("w", encoding="utf-8") as f:
