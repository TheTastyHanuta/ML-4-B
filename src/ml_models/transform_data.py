...
# Datensatz laden
#df = pd.read_csv('../data/data-2025-01.csv')
#df = pd.read_parquet("../../data/bahn_data", engine="pyarrow")
# Read all data from the last 4 full months
df = pd.concat([pd.read_parquet(f, engine="pyarrow") for f in sorted(Path("../../data/bahn_data").glob("*.parquet"))[-4:]], ignore_index=True)
# Nur ICE
...
print(df_subtrips.head(10))
# Speichern des DataFrames als CSV
#df_subtrips.to_csv('../data/subtrips_2025-01.csv', index=False, encoding='utf-8')
# Speichern des DataFrames als Parquet
df_subtrips.to_parquet(Path("../../data/bahn_data/processed/subtrips_data.parquet"), index=False, engine='pyarrow')
